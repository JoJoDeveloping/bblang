...
pub fn ptradd(v1: &Rc<Value>, v2: &Rc<Value>) -> Rc<Value> {
    match (&**v1, &**v2) {
        (Value::PtrValue(n1), Value::NumValue(n2)) => {
            Rc::new(Value::PtrValue(InfPointer(n1.0, n1.1.clone() + n2)))
        }
...
pub fn ptrdiff(v1: &Rc<Value>, v2: &Rc<Value>) -> Rc<Value> {
    match (&**v1, &**v2) {
        (Value::PtrValue(InfPointer(o1, n1)), Value::PtrValue(InfPointer(o2, n2))) if o1 == o2 => {
